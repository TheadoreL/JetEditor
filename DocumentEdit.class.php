...
class documentedit{
	...
	private function RecoMainArray($displayArray){
		...
		foreach ($displayArray as $key => $value) {
			...
			$MainArray[$key]['input_name'] = $displayArray[$key]['id'];
		}
		return $MainArray;
	}
	//整理数据库名
	...
	private function GainArray($table_id,$limits,$jud){
		$db = new db_pdo();
		if ($jud == '0') {
			$sql = "select * from form_model where table_id = :table_id and edit_limits >= :limits_start and edit_limits <= :limits_end";
			$limit = explode(",", $limits);
			$a = array("table_id" => $table_id,"limits_start" => $limit[0],"limits_end" => $limit[1]);
		}
		elseif ($jud == '1') {
			$sql = "select * from ".$table_id." where id = :limits";
			$a = array("limits" => $limits);
		}
		$re = $db->query($sql,$a);
		return $re;
	}
